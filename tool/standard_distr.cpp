...
namespace StandardDistributions {
  ...
  unique_ptr<PrositAux::pmf> FileDistrBuilder::create_instance(DistrFactory::DistrParameters * t) throw (Exc) {
    FileDistrParameters *pt;
    if (!(pt = dynamic_cast<FileDistrParameters*>(t)))
      EXC_PRINT("worng parameter type");

    unique_ptr<PrositAux::pmf> c (new PrositAux::pmf (pt->size,0));
    c->load(pt->fileName);
    return c;
  };
  ...
  unique_ptr<PrositAux::pmf> UniformDistrBuilder::create_instance(DistrFactory::DistrParameters * t) throw (Exc) {
  ...
  unique_ptr<PrositAux::pmf> BetaDistrBuilder::create_instance(DistrFactory::DistrParameters * t) throw (Exc) {
